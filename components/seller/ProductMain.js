...
export default function ProductMain() {
  ...
  const { data, error, mutate } = useSWR(
    userId ? `/api/seller/${userId}/products` : null,
    fetcher
  );
  ...
  const handleDelete = async (id) => {
    ...
    await fetch(`/api/seller/${userId}/products/${id}`, { method: "DELETE" });
    mutate();
  };
  ...
  const handleBulkDelete = async () => {
    ...
    await Promise.all(
      selected.map((id) =>
        fetch(`/api/seller/${userId}/products/${id}`, { method: "DELETE" })
      )
    );
    mutate();
    setSelected([]);
  ...
  const filteredProducts = products
    .filter((p) => {
      ...
      const catFilter = category === "all" || p.category === category;
      return match && stockFilter && catFilter;
    })
    .sort((a, b) => {
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          ...
          <Link href={`/seller/${userId}/products/create`}>
      ...
      {filteredProducts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProducts.map((p) => (
            <div key={p._id} className="bg-white shadow-lg rounded-2xl overflow-hidden flex flex-col">
              <img
              ...
              <div className="p-4 flex-1 flex flex-col justify-between">
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="space-x-2">
                    ...
                    <Link href={`/seller/${userId}/products/${p._id}`}>
        ...
        <div className="space-y-4">
          {filteredProducts.map((p) => (
            <div key={p._id} className="bg-white shadow rounded-xl p-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <input type="checkbox" checked={selected.includes(p._id)} onChange={() => toggleSelect(p._id)} />
                <img
                  src={p.images?.[0] || 'https://via.placeholder.com/100x100?text=No+Image'}
                  alt={p.name}
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => handleDelete(p._id)} className="text-red-600 hover:underline text-sm">
                  Delete
                </button>
              </div>
            </div>

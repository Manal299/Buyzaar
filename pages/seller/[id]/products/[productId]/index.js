...
export default function EditProductPage() {
  ...
  const { id: userId, productId } = router.query;
  ...
  useEffect(() => {
    async function fetchProduct() {
      const res = await fetch(`/api/seller/${userId}/products/${productId}`);
  ...
  <div>
    ...
    <input
      ...
      onChange={(e) =>
        setForm({ ...form, colors: e.target.value.split(",").map((c) => c.trim()) })
      }
      className="w-full border px-4 py-2 rounded"
    />
  </div>
  ...
  <div>
    <label className="block font-medium mb-1">Internal Inventory Count</label>
    <input
      type="number"
    ...
    <div className="grid grid-cols-2 gap-2">
      {form.images.map((url, i) => (
        <div key={i} className="relative">
          <img src={url} alt={`Uploaded ${i + 1}`} className="h-32 w-full object-cover rounded-lg border" />
          <button
            type="button"
